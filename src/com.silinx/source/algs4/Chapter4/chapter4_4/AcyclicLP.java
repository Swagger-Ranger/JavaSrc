 ...
 *
 *  和 AcyclicSP 相反的求最长路径的方法，本质上和AcyclicSP一致，只是初始化时将值变为最小（即负无穷大）和放松方法的反过来
 *************************************************************************/
...
public class AcyclicLP {
    ...
    public AcyclicLP(EdgeWeightedDigraph G, int s) {
        ...
        for (int v = 0; v < G.V(); v++) distTo[v] = Double.NEGATIVE_INFINITY; //相反点1
        distTo[s] = 0.0;
    ...
    private void relax(DirectedEdge e) {
        int v = e.from(), w = e.to();
        if (distTo[w] < distTo[v] + e.weight()) {  //相反点2
