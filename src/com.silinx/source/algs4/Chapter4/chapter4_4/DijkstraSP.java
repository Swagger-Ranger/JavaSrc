 ...
 *
 *  最短路径--迪杰斯特拉算法
 *  最短路径的最优性条件：任意节点v联通到任意节点w的边的权重e，都必须满足distTo[w] <= distTo[v] + e
 *  和prim算法结构，思想基本一致：都是从已有的节点出发贪心算法，每一步都寻找添加最短的路径
 *                             差别在于关键方法prim（）-->relax（），同时不需要标记一检查过的点而是检查每条边，检查点-->检查边
 *  关键方法：relax
 *************************************************************************/

public class DijkstraSP {
    private double[] distTo;          // distTo[v] = distance  of shortest s->v path  起点到v的距离或者是权重
    ...
    public DijkstraSP(EdgeWeightedDigraph G, int s) {
        ...
        for (int v = 0; v < G.V(); v++)
            distTo[v] = Double.POSITIVE_INFINITY;  //将所有未检查的节点都初始化为无穷大
        distTo[s] = 0.0;
