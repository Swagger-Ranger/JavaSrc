...
import com.silinx.source.algs4.Chapter4.chapter4_2.Topological;
 ...
 *
 *  无环有向加权图的最短路径，先将无环图进行拓扑排序，按照拓扑排序来进行放松边即relax()方法；就能得到最短路径
 *  这个方法是最快最优的解法，因为：按照拓扑排序进行方松时，e：v-w,前面放松的点得到的距离必定是不变的，因为拓扑排序使得不可能再回到前面的点，图是
 *                              无环的，那么放松边e只能变地越来越小，所以在方松完所有的边后得到的路径就必然是最小的
 *************************************************************************/
