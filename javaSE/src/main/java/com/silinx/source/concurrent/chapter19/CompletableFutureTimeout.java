...
public class CompletableFutureTimeout {
    ...
    static final class Delayer {
        ...
        static final class DaemonThreadFactory implements ThreadFactory {
            ...
            public Thread newThread(Runnable r) {
                ...
                // 因为使用线程来做执行 ScheduledThreadPoolExecutor 中的后台任务，如果没有其他任务则退出，所以设置为守护线程；
                // 否则程序存在非守护线程会不退出
                t.setDaemon(true);
        ...
        static {
            ...
            // 如果任务取消，则将任务从等待队列中移除
            // https://stackoverflow.com/questions/69107355/when-to-use-scheduledthreadpoolexecutors-removeoncancel-policy
    ...
    public static <T> CompletableFuture<T> timeoutAfter(long timeout, TimeUnit unit) {
        ...
        ScheduledFuture<?> delay = Delayer.delay(() -> result.completeExceptionally(new TimeoutException()), timeout, unit);
        // delay可以选择 cancel(true) 来取消任务成功后 delayer.setRemoveOnCancelPolicy(true) 队列中的未执行任务就会理解删除
        return result;
    }
    ...
    private static void test() throws ExecutionException, InterruptedException {
        ...
        CompletableFuture<String> withinStr = CompletableFutureTimeout.orTimeout(futureStr, 1, TimeUnit.SECONDS);
