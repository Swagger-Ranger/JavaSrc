 ...
 *  11: 4 12 
 *  12: 9
 *  有向图的实现，和无向图基本类似，只是在添加边时只需要添加一边，不用相互添加
 *  
 ******************************************************************************/
...
public class Digraph {
    ...
    private int[] indegree;        // indegree[v] = indegree of vertex v  入度，指向此节点的节点数量
     ...
     *
     * @param  G the digraph to copy拷贝另一个有向图
     */
    public Digraph(Digraph G) {
        ...
        for (int v = 0; v < G.V(); v++) {
            ...
            Stack<Integer> reverse = new Stack<Integer>();  //在拷贝另一条边时要考虑方向所以需要转向添加
            for (int w : G.adj[v]) {
                reverse.push(w);
            }
            for (int w : reverse) {
    ...
    public void addEdge(int v, int w) {
        ...
        validateVertex(w);
        adj[v].add(w);
        indegree[w]++;//这里需要维护一个入度，出度就等于节点本身的大小（包的大小）
        E++;
    }
