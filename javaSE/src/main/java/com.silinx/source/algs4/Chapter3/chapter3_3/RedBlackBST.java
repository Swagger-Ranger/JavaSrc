package com.silinx.source.algs4.Chapter3.chapter3_3;
 ...
 *  S 0
 *  X 7
 *  红黑树：含有一个color的boolean变量来将二叉查找树（左小右大的类似于快速排序结构的二叉树）的链接分成了红黑两类的二叉查找2-3树
 *  红黑树的定义：--这里的定义是一下3条，和其它的定义可能有区别，但关键并不是定义的细节而是这种数据结构的本质是：
 *              实现了查找，插入，删除，查找最大/小，删除最大/小，选择和范围选择等操作的时间所需都为对数级别lgN的一种二叉树。
 *              这里怎么定义有些原因是实现起来更方便比如定义1也可以是允许有右子树为红；当然也可以增加一下定义，比如所有的底子树都必须为黑
 *      1.红链接均为左链接
 *      2.没有任何的节点同时拥有两个红链接
