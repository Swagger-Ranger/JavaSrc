 ...
 *  相比LSD只能排序等长的字符串，MSD则可以排序不等长的字符串
 ******************************************************************************/
...
public class MSD {
   ...
   /**
    ...
    //这里有个特例：当字符位置等于了字符串的长度时就返回-1（之后会+1=0)，同时不可能出现超过字符串长度的情况，因为递归不会去执行前面已经排序完成了的子数组
    ...
    private static void sort( String[] a, int lo, int hi, int d, String[] aux) {
        ...
        // cutoff to insertion sort for small subarrays将较小的数组切分到插入排序，以避免在处理小数组时影响性能
        if (hi <= lo + CUTOFF) {
            insertion(a, lo, hi, d);
            return;
        }
        ...
        int[] count = new int[R+2];     //--这里就是影响性能的关键，如果每次都去递归小数组那么这里每次都会去初始化一个R基数大小的数组
        for (int i = lo; i <= hi; i++) {
            int c = charAt(a[i], d);
            count[c+2]++;
        }
